...
it('Design constructor should handle Simon', () => {
  ...
  let pattern = new design()
  expect(pattern.config.draftOrder[0]).to.equal('base')
  expect(pattern.config.draftOrder[1]).to.equal('frontBase')
  expect(pattern.config.draftOrder[2]).to.equal('front')
  expect(pattern.config.draftOrder[3]).to.equal('backBase')
  expect(pattern.config.draftOrder[4]).to.equal('back')
  expect(pattern.config.draftOrder[5]).to.equal('sleeveBase')
  expect(pattern.config.draftOrder[6]).to.equal('sleeve')
  expect(pattern.config.draftOrder[7]).to.equal('frontRight')
  expect(pattern.config.draftOrder[8]).to.equal('frontLeft')
  expect(pattern.config.draftOrder[9]).to.equal('buttonPlacket')
  expect(pattern.config.draftOrder[10]).to.equal('buttonholePlacket')
  expect(pattern.config.draftOrder[11]).to.equal('yoke')
  expect(pattern.config.draftOrder[12]).to.equal('collarStand')
  expect(pattern.config.draftOrder[13]).to.equal('collar')
  expect(pattern.config.draftOrder[14]).to.equal('sleevePlacketUnderlap')
  expect(pattern.config.draftOrder[15]).to.equal('sleevePlacketOverlap')
  expect(pattern.config.draftOrder[16]).to.equal('cuff')
})
